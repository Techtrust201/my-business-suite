...
const hasDiscounts = (lines: DocumentLine[]): boolean => {
  return lines.some(line => line.discount_percent && line.discount_percent > 0);
};
...
const addHeader = async (
  ...
  if (organization.postal_code || organization.city) {
    addressParts.push(`${organization.postal_code || ''} ${organization.city || ''}`.trim());
  }
  ...
  doc.text(documentType, badgeX + badgeWidth / 2, yPos + 5, { align: 'center' });
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(...COLORS.gray);
  doc.text(`Date: ${format(new Date(documentDate), 'dd/MM/yyyy')}`, pageWidth - 15, yPos + 19, { align: 'right' });
  
  if (dueDate) {
    doc.text(`Échéance: ${format(new Date(dueDate), 'dd/MM/yyyy')}`, pageWidth - 15, yPos + 24, { align: 'right' });
  }
  if (validUntil) {
    doc.text(`Valide jusqu'au: ${format(new Date(validUntil), 'dd/MM/yyyy')}`, pageWidth - 15, yPos + 24, { align: 'right' });
  }
  ...
  if (subject || purchaseOrderNumber) {
    ...
    if (subject) {
      ...
      const truncatedSubject = subject.length > 80 ? subject.substring(0, 77) + '...' : subject;
...
const addClientInfo = (doc: jsPDF, contact: Contact | null | undefined, yPos: number): number => {
  ...
  if (contact.billing_address_line1) {
    const truncatedAddr = contact.billing_address_line1.length > 35 ? contact.billing_address_line1.substring(0, 32) + '...' : contact.billing_address_line1;
    doc.text(truncatedAddr, boxX + 4, clientY);
    clientY += 3.5;
  }
  if (contact.billing_postal_code || contact.billing_city) {
    doc.text(`${contact.billing_postal_code || ''} ${contact.billing_city || ''}`.trim(), boxX + 4, clientY);
    clientY += 3.5;
  }
  if (contact.email) {
    const truncatedEmail = contact.email.length > 30 ? contact.email.substring(0, 27) + '...' : contact.email;
...
const addLinesTable = (doc: jsPDF, lines: DocumentLine[], yPos: number): number => {
  ...
  const columnStyles: Record<number, { halign?: 'left' | 'right' | 'center'; cellWidth?: number | 'auto' }> = showDiscount 
    ? {
        0: { cellWidth: 'auto' },
...
const addTotalsWithVat = (
  ...
  doc.setTextColor(...COLORS.dark);
  doc.text(formatPrice(subtotal), boxX + boxWidth - 4, currentY, { align: 'right' });
  currentY += rowHeight;
  ...
  vatSummary.forEach(v => {
    ...
    doc.setTextColor(...COLORS.dark);
    doc.text(formatPrice(v.vat), boxX + boxWidth - 4, currentY, { align: 'right' });
    currentY += rowHeight;
  });
  ...
  doc.text(formatPrice(total), boxX + boxWidth - 4, currentY + 2, { align: 'right' });
  ...
  if (amountPaid && amountPaid > 0) {
    ...
    doc.text(`-${formatPrice(amountPaid)}`, boxX + boxWidth - 4, currentY, { align: 'right' });
    currentY += rowHeight;
    ...
    doc.text(formatPrice(total - amountPaid), boxX + boxWidth - 4, currentY, { align: 'right' });
  }
...
const addBankInfo = (doc: jsPDF, organization: Organization, yPos: number): number => {
  ...
  if (organization.bank_details) bankParts.push(organization.bank_details);
  if (organization.rib) bankParts.push(`IBAN: ${organization.rib}`);
  if (organization.bic) bankParts.push(`BIC: ${organization.bic}`);
  
  const bankText = bankParts.join(' • ');
  const maxWidth = pageWidth - 55;
  const truncatedBank = doc.splitTextToSize(bankText, maxWidth)[0];
  doc.text(truncatedBank, 38, yPos);
...
const addTermsAndNotes = (doc: jsPDF, terms?: string | null, notes?: string | null, yPos?: number): number => {
  ...
  if (terms) {
    ...
    const limitedTerms = splitTerms.slice(0, 2).join(' ');
    doc.text(limitedTerms, 38, currentY);
    
    currentY += splitTerms.length > 1 ? 7 : 4;
  }
  ...
  if (notes) {
    ...
    const limitedNotes = splitNotes.slice(0, 2).join(' ');
    doc.text(limitedNotes, 30, currentY);
    
    currentY += splitNotes.length > 1 ? 7 : 4;
  }

  return currentY;
};

const addLegalMentions = (doc: jsPDF, organization: Organization, yPos?: number): void => {
  const pageWidth = doc.internal.pageSize.getWidth();
  ...
  if (organization.vat_number) identifiers.push(`TVA Intra.: ${organization.vat_number}`);
  if (organization.website) identifiers.push(organization.website);
  
  if (identifiers.length > 0) {
    doc.text(identifiers.join(' • '), pageWidth / 2, footerY, { align: 'center' });
  }
  
  // Legal mentions
  if (organization.legal_mentions) {
    const splitMentions = doc.splitTextToSize(organization.legal_mentions, pageWidth - 30);
...
export const generateInvoicePDF = async (invoice: Invoice, organization: Organization): Promise<jsPDF> => {
  ...
  yPos = addTotalsWithVat(doc, lines, subtotal, taxAmount, total, yPos, amountPaid);
